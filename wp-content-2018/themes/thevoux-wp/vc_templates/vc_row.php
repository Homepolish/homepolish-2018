<?php
$output = $el_class = $bg_image = $bg_color = $bg_image_repeat = $font_color = $padding = $margin_bottom = $css = $parallax_class = $midnight = $mouse = '';
...
// Color Change
if ($section_color == 'dark-bg') {
$midnight = ' data-midnight="'.$section_color.'"';	
}
$output .= '<div '.($row_id ? 'id="'.$row_id.'"' : '') .' class="row '.$mouse.$full_width.$nopadding.$parallax_class.$video.$height.$el_class.vc_shortcode_custom_css_class($css, ' ').'" '.$data.$equal_height.$midnight.'>';
$output .= $bg_video;
