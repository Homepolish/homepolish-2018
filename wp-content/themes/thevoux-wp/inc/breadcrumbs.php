...
function thb_which_archive() {
	...
	if ( is_category() )
	{
		$output = __('Archive for category:', THB_THEME_NAME)." <span>".single_cat_title('',false)."</span>";
	}
	elseif (is_day())
	{
		$output = __('Archive for date:', THB_THEME_NAME)." <span>".get_the_time('F jS, Y')."</span>";
	}
	elseif (is_month())
	{
		$output = __('Archive for month:', THB_THEME_NAME)." <span>".get_the_time('F, Y')."</span>";
	}
	elseif (is_year())
	{
		$output = __('Archive for year:', THB_THEME_NAME)." <span>".get_the_time('Y')."</span>";
	}
	elseif (is_search())
	{
		global $wp_query;
		if(!empty($wp_query->found_posts))
		{
			...
			{
				$output =  __('Search results for', THB_THEME_NAME)." <h5>".esc_attr( get_search_query() )."</h5>";
			}
			else
			{
				$output =  __('Search result for', THB_THEME_NAME)." <h5>".esc_attr( get_search_query() )."</h5>";
			}
		}
		else
		{
	...
	elseif (is_author())
	{
		...
		if(isset($curauth->nickname)) $output .= __('<em>for</em>', THB_THEME_NAME)." ".$curauth->nickname;

	}
	elseif (is_tag())
	{
		$output = __('Tag Archive <em>for</em>', THB_THEME_NAME)." <span>".single_tag_title('',false)."</span>";
	}
	elseif(is_tax())
	{
		$term = get_term_by( 'slug', get_query_var( 'term' ), get_query_var( 'taxonomy' ) );
		$output = __('Archive <em>for</em>', THB_THEME_NAME)." <span>".$term->name."</span>";
	}
	else
	{
	...
	{
		$output .= " (".__('Page', THB_THEME_NAME)." <span>".$_GET['paged'].")"."</span>";
	}
